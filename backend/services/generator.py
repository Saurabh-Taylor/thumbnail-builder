...
async def generate_single_thumbnail(thumbnail_id: str, prompt: str, headshot_url: str):
    # DB connection --> will mark as generating
    with Session(engine) as session:
        thumb = session.get(Thumbnails, thumbnail_id)
...
async def process_job(job_id: str):
    ...
    # find all the thumbnails for this job
    # start one worker for each thumbnail 
    # wait for all workeers to finish
    ...
    thumbnails = session.exec(
        select(Thumbnails).where(thumbnails.job_id == job_id)
    )
